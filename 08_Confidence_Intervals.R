...
rels$score <- rels$obs / rels$e11^0.78
rels$scoreu <- rels$obsu / rels$e11u^0.78
rels$scorel <- rels$obsl / rels$e11l^0.78

rels |> 
  filter(focal == "слеза") |> 
  arrange(-score) |> 
  slice_head(n = 10) |> 
  ggplot(aes(reorder(bound, score), score, color = bound)) +
